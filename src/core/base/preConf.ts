	...
	try {
		...
		if (!_arr.includes(LICENSE_PATH)) {
			...
			if (_arr.includes(MAIN_FILE_NAME)) {
				const _arrOther = _arr.filter((v: string) => v !== MAIN_FILE_NAME);
				...
				_obj.addr = objFileMain.addr.map((v: string) => v || 'localhost')[_obj.isDev ? 0 : 1];
				...
				for (const v of _arrOther) {
					if (v.endsWith('.js')) {
						...
						for (const kConf in otherConf) {
							if (kConf === 'dbAddr') {
								// @ts-ignore
								_objUnit['dbAddr'] = otherConf ? otherConf[kConf].map((v: string) => v || 'localhost')[_obj.isDev ? 0 : 1] : _obj.addr;
							} else {
								// @ts-ignore
