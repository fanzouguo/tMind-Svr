(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(require('tmind-core'), require('ws')) :
	typeof define === 'function' && define.amd ? define(['tmind-core', 'ws'], factory) :
	(global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.tmindSvr = factory(global.tmindCore, global.WebSocket));
}(this, (function (tmindCore, WebSocket) { 'use strict';
	...
	var preConf = (currPath) => {
	    try {
	        ...
	        if (!_arr.includes(LICENSE_PATH)) {
	            ...
	            let i = 1;
	            let tempObj = {};
	            if (_arr.includes(MAIN_FILE_NAME)) {
	                const _arrOther = _arr.filter((v) => v !== MAIN_FILE_NAME);
	                const objFileMain = require(`${rootPath}/${MAIN_FILE_NAME}`);
	                _obj.id = objFileMain.id;
	                _obj.ident = pjIdent;
	                _obj.namezh = objFileMain.namezh || terminat('工程名称配置项不允许为空', MSG_BOOT_ERR, 1);
	                _obj.addr = objFileMain.addr.map((v) => v || 'localhost')[_obj.isDev ? 0 : 1];
	                _obj.secretKey = objFileMain.secretKey || terminat('加盐码不能为空', MSG_BOOT_ERR, 1);
	                for (const v of _arrOther) {
	                    if (v.endsWith('.js')) {
	                        ...
	                        for (const kConf in otherConf) {
	                            if (kConf === 'dbAddr') {
	                                // @ts-ignore
	                                _objUnit['dbAddr'] = otherConf ? otherConf[kConf].map((v) => v || 'localhost')[_obj.isDev ? 0 : 1] : _obj.addr;
	                            }
	                            else {
	                                // @ts-ignore
	                ...
	                return terminat('配置文件夹主文件丢失!', MSG_BOOT_ERR, 0);
	            }
	        }
	    }
	    catch (err) {
	...
	class TimeTask {
	     ...
	     */
	    constructor(currPath, conf, ident) {
	        this.#ident = ident;
...
IP.2 = 127.0.0.1`;
	...
	class BaseSvr extends EventEmitter {
	    constructor(appDir) {
	        super();
	        this.showLog = false;
	    ...
	    connectToLog() {
	        return new Promise((resolve, reject) => {
	            ...
	            if (this.ident !== 'log') {
	                ...
	                this.#logger.on('open', () => {
	                    this.setInfo('已连接到日志服务', 11101 /* Svr_Boot */, -1, 'boot');
	                    resolve('');
	                });
	    ...
	    setInfo(msg, currLogType, reqId, tag, title, msgType = 'INFO') {
	        if (this.ident !== 'log') {
	            ...
	            tmindCore.tEcho('日志服务器不支持 setInfo 方法', '警告！', 'WARN');
	        }
	    }
	    /** 创建WARN类日志
	     * @param msg 要写入日志的信息文本
	    ...
	    setWarn(msg, currLogType, reqId, tag) {
	        if (this.ident !== 'log') {
	            ...
	            tmindCore.tEcho('日志服务器不支持 setWarn 方法', '警告！', 'WARN');
	        }
	    }
	    /** 创建ERR类日志
	     * @param msg 要写入日志的信息文本
	    ...
	    setErr(err, currLogType, reqId, tag) {
	        if (this.ident !== 'log') {
	            ...
	            tmindCore.tEcho('日志服务器不支持 setErr 方法', '警告！', 'WARN');
	        }
	    }
	    async http(urlOrOpt, optOrMethod) {
	        ...
	        try {
	            ...
	            this.setErr(err, 13005 /* Svr_Http_Request_Err */, -1, 'Svr Http');
	        }
	    }
	    /** 启动服务
	     *
	    ...
	    async start() {
	        try {
	            tmindCore.tEcho('连接日志服务...', '准备中', 'INFO');
	            await this.connectToLog();
	            tmindCore.tEcho('日志服务已连接', 'Next', 'SUCC');
	            this.#TimeTask.start();
	            this.#timTaskStoped = false;
