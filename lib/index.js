...
IP.2 = 127.0.0.1`;
	...
	class SvrBase extends EventEmitter {
	    constructor(appDir) {
	        ...
	        const objLogger = new WebSocket__default['default'](this.configAll.loggerUrl);
	        objLogger.on('open', (ws) => {
	            this.#logger = ws;
	            ws.send({}, (err) => {
	                this.exit('启动日志写入失败，本服务也将随之终止.');
	            });
	            this.setLog('INFO', `[${this.config.namezh}]服务已启动`, 11000 /* Svr_Boot */, -1, '启动');
	        });
	        objLogger.on('close', () => {
	            this.exit('日志服务已终止，本服务也将随之终止.');
	        });
	        objLogger.on('error', (err) => {
	    ...
	    echo(msg, title, msgType) {
	        ...
	        if (!title) {
	            ...
	            tmindCore.tEcho(msg, title, msgType);
	        }
	    }
	    preErr(a, b, c) {
	        const _errObj = (((typeof a === 'string') && new Error(a)) || a);
	        const isBoolB = typeof b === 'boolean';
	        if (b) {
	            _errObj.code = !isBoolB ? b : 13000 /* Unkown_ERR */;
	        }
	        else {
	            _errObj.code = 13000 /* Unkown_ERR */;
	        }
	        if (isBoolB || c) {
	            tmindCore.tEcho(typeof a === 'string' ? a : a.message, '异常', 'ERR');
	        }
	     ...
	     * @param tag 日志消息或
	     */
	    setLog(msgType, msg, currLogType, reqId, tag) {
	        if (this.#logger) {
	            this.#logger.send({
	                msgType,
	                msg,
	    ...
	    start() {
	        try {
	            ...
	            tmindCore.tEcho(`配置指向：${this.addr}`, '', 'INFO');
