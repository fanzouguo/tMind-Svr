...
IP.2 = 127.0.0.1`;
	    ...
	    connectToLog() {
	        return new Promise((resolve, reject) => {
	            ...
	            if (this.ident !== 'log') {
	                ...
	                this.#logger.on('open', () => {
	                    this.setInfo('日志服务已连接', 11101 /* Svr_Boot */, -1, 'boot');
	                    resolve('');
	                });
	...
	// 分隔符
	const splitSymbo = '.';
	const getFileName = () => tmindCore.tDate().format('YYYY-MM-DD');
	const getItemKey = () => tmindCore.tDate().format('.YYYY年.MM月.DD日.hh时.mi分');
	const getLogTime = () => tmindCore.tDate().format('YYYY-MM-DD hh:mi:ss.ms');
	class LogSvr$1 extends SvrBase {
	    constructor(appDir) {
	        super(appDir);
	        ...
	        svr.on('connection', (ws) => {
	            let _logFrom = '';
	            ws.on('message', (msg) => {
	                const _obj = JSON.parse(msg.toString());
	                if (!_logFrom) {
	                    _logFrom = _obj.from;
	                }
	    ...
	    async start() {
	        this.setTimeTask(() => {
	            try {
	                ...
	                tmindCore.tEcho('日志存盘失败，详情如下：', '异常', 'ERR');
	                tmindCore.tEcho(err, '', 'ERR');
	            }
	        });
	        super.start();
	...
	const LogSvr = LogSvr$1;
	const DbSvr = DbSvr$1;

	exports.DbSvr = DbSvr;
	exports.KoaSvr = KoaSvr;
	exports.LogSvr = LogSvr;
