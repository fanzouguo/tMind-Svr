import { tEcho, smpoo, tDate } from 'tmind-core';
...
class BaseSvr extends EventEmitter {
    ...
    async start() {
        try {
            ...
            if (this.ident !== 'log') {
                this.setInfo(`[${this.config.namezh}] Server is start at ${this.config.addr}:${this.config.port} in ${tDate().format('YYYY-MM-DD hh:mi:ss.ms')}`, 11101 /* Svr_Boot */, -1, 'boot', '', 'SUCC');
            }
        }
        catch (err) {
            tEcho(err, '启动失败', 'ERR');
